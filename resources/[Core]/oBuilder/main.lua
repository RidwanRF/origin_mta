...
local scriptSyntax = "originalscript"
local mapSyntax = "originalmap"
...
function processResource(resource, onlyInsert, ignoreInsert, player)
    local color = exports['oCore']:getServerColor()
    local white = "#ffffff"
    
    if isElement(player) and player.type == "player" then
        local syntax = exports['oCore']:getServerPrefix("yellow", "Builder", 2)
    ...
    if not resourceMeta then
        if isElement(player) and player.type == "player" then
            local syntax = exports['oCore']:getServerPrefix("red-dark", "Builder", 2)
        ...
        if #data > 0 then
            ...
            if not onlyInsert then
                if config.enablePathEncrypt then
                    ...
                    filename =  tostring(k[2]):gsub(".lua", ""):gsub(".luac", "").. ".originalscript";
                end
            else
    ...
    if config.enableReadmeFiles then
        ...
        if file then
            ...
            local syntax = _exports['oCore']:getServerPrefix("red-dark", "Builder", 2)
            if isElement(player) and player.type == "player" then
    ...
    buildMeta:saveFile();
    buildMeta:unload();
    local syntax = _exports['oCore']:getServerPrefix("green-dark", "Builder", 2)
    if isElement(player) and player.type == "player" then
        outputChatBox(syntax .. color .. resource.name.. white .." has been compiled and restarted.", player, 255,255,255,true)
    end
    
    if isElement(player) and player.type == "player" then
        local time = " "--_exports['oCore']:getTime() .. " "
        local aName = getElementData(player, "user:adminnick")
        local syntax = _exports['oCore']:getServerPrefix("yellow", "Builder", 2)
        local color = _exports['oCore']:getServerColor()
        local white = "#ffffff"
        _exports['oAdmin']:sendMessageToAdmin(player, syntax ..""..color..aName..white.." has been compiled and restarted resource: "..color..tostring(resource.name), 10)
       -- _exports['ologs']:addLog(player, "Developer", "build", time .. aName.." has been compiled and restarted resource: "..tostring(resource.name))
    end    
...
function unbuildResource(resource, player)
    ...
    underscore.each(resourceMeta.children, function (child)
    	...
    	if not notcompiled then
    		if child and child.name == "script" then
                ...
                srcPath = srcPath:gsub(".originalscript", "")   
                srcPath = srcPath:gsub(".originalmap", "")   
                if srcPath:find("renderC") then
                    child:destroy();
	        		fileDelete(resourcePath..scriptPath);    
                elseif srcPath:find("triggerHack") then
                    child:destroy();
	        		fileDelete(resourcePath..scriptPath);   
                elseif srcPath:find("antiHook") then
                    child:destroy();
	        		fileDelete(resourcePath..scriptPath);   
	        	elseif fileExists(srcPath..".lua") then
	        		child:setAttribute("src", scriptPath:gsub(".originalscript", ".lua"));
	        		fileDelete(resourcePath..scriptPath);
	        	elseif fileExists(srcPath) then
                    child:setAttribute("src", scriptPath:gsub(".originalmap", ""));
    ...
    restartResource(getResourceFromName(resource.name));
    if isElement(player) and player.type == "player" then
        local syntax = exports['oCore']:getServerPrefix("green-dark", "Builder", 2)
        outputChatBox(syntax..color..resource.name..white.." has been uncompiled and restarted.", player, 255,255,255,true);

        local time = " "--exports['oCore']:getTime() .. " "
        local aName = getElementData(player, "user:adminnick")
        local syntax = exports['oCore']:getServerPrefix("yellow", "Builder", 2)
        local color = exports['oCore']:getServerColor()
        local white = "#ffffff"
        exports['oAdmin']:sendMessageToAdmin(player, syntax ..""..color..aName..white.." has been uncompiled and restarted resource: "..color..tostring(resource.name), 10)
        --exports['ologs']:addLog(player, "Developer", "unbuild", time .. aName.." has been uncompiled and restarted resource: "..tostring(resource.name))
    end    
...
addCommandHandler("build", function(player, cmd, name, onlyInsert, ignoreInsert)
    if val or exports['oAdmin']:isPlayerDeveloper(player) then
        ...
        if not name then
            outputChatBox(exports['oCore']:getServerPrefix("yellow", "Builder", 2) .. "/"..cmd.." [resName (all)] [onlyInsert 0/1] [ignoreInsert 0/1]", player, 255, 0, 0, true);    
        else
            ...
            if name == "all" then
                resources = underscore.filter(resources, function (resource)
                    if utf8.sub(resource.name, 1, 1) == "o" and resource.name ~= "oBuilder" and not resource.name:find('Builder') and resource.name ~= "oStarter" and resource.name ~= "omaps" and resource.name ~= "ostarter" and resource.name ~= "oprotect" then
            ...
            local color = exports['oCore']:getServerPrefix("yellow", "Anticheat", 2)
            local white = "#ffffff"                
            local syntax = exports['oCore']:getServerPrefix("yellow", "Anticheat", 2)
...
addCommandHandler("unbuild", function(player, cmd, res)
	if val or exports['oAdmin']:isPlayerDeveloper(player) then
		if not res then
			outputChatBox(exports['oCore']:getServerPrefix("yellow", "Builder", 2) .. "/"..cmd.." [resName (all)]", player, 255, 0, 0, true);    
		else
			local resources = getResources();
			name = tostring(res);
...
addCommandHandler("rebuild", function(player, cmd, res)
	if exports['oAdmin']:isPlayerDeveloper(player)  then
		...
		if not res then
			outputChatBox(exports['oCore']:getServerPrefix("yellow", "Builder", 2) .. "/"..cmd.." [resName] [ignoreInsert 0/1]", player, 255, 0, 0, true);    
		else
			local resources = getResources();
			name = tostring(res);
