    ...
    if getElementData(localPlayer, "user:admin") >= 7 then 
        ...
        if num then
            if not isElement(shader) then return end
                ...
                if fileExists("paintjobs/"..getElementModel(veh).."/"..num..".png") then 
...
function applyPaintjobToVehicle(veh, id)
    ...
    if not paintsjobs[veh] then paintsjobs[veh] = dxCreateShader(shader_data) end
    ...
    if not textures[model][id] then 
        textures[model][id] = dxCreateTexture("paintjobs/"..model.."/"..id..".png", "dxt1")
    end
...
function removePaintjobFromVehicle(veh)
    if paintsjobs[veh] then
        ...
        if customTextures[model] then 
            engineRemoveShaderFromWorldTexture(paintsjobs[veh], customTextures[model], veh)
        else
            engineRemoveShaderFromWorldTexture(paintsjobs[veh], "*remap*", veh)
        end
    end
end
