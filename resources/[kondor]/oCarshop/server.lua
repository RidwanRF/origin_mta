...
function buyVehicle(modelId, color, owner, price_dollar, price_premium, shopType, city)
    ...
    if firstEmpyCol then
        ...
        if (price_dollar > 0) then
            if (price_dollar <= getElementData(owner,'char:money')) then
                local veh = vehicle:createNewVehicle(modelId, owner, {px, py, pz}, {0, 0, rot}, color)
        
                setElementRotation(veh, 0, 0, rot)
                warpPedIntoVehicle(owner, veh)
                ...
                infobox:outputInfoBox("Sikeresen megvásároltad a kiválasztott járművet!","success",owner)
            else
                infobox:outputInfoBox("Nincs elegendő készpénzed a vásárláshoz! ("..price_dollar.."$)","error",owner)
            end
        elseif (price_premium > 0) then
            if (price_premium <= getElementData(owner,'char:pp')) then
                local veh = vehicle:createNewVehicle(modelId, owner, {px, py, pz}, {0, 0, rot}, color)
                ...
                infobox:outputInfoBox("Nincs elegendő prémium pontod a vásárláshoz! ("..price_premium.."PP)","error",owner)
            end
        end
