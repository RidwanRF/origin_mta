...
func.useItem = function(slot,data)
	...
	elseif data.item == 208 then
		...
		if occupiedVeh then
			if getElementData(localPlayer, "char:money") >= 2500 then
				if getElementData(occupiedVeh, "veh:isFactionVehice") == 1 then
					...
					if ownerName and core:getDistance(element, localPlayer) <= 40 then
						if getElementHealth(occupiedVeh) >= 990 then
							setElementData(localPlayer, "char:money", getElementData(localPlayer, "char:money") - 2500)
							pdaInUse = true
				...
				outputChatBox(core:getServerPrefix("red-dark", "OBD Scanner", 2).."Nincs nálad elegendő pénz!", 255, 255, 255, true)
			end
		else
	...
	elseif data.item == 227 then
		if cache.inventory.active.identity == -1 or cache.inventory.active.identity == slot then
			if getElementData(localPlayer, "char:duty:faction") == 2 then
