...
function renderPanel() 
    ...
    if getElementData(shopElement, "shop:private") == 1 then 
        if getElementData(shopElement, "shop:owner") == 0 then 
            ...
            dxDrawText("Ez a bolt eladó "..color..getElementData(shopElement, "shop:cost").."$#ffffff-ért. Szeretnéd megvásárolni?", sx*0.4, sy*0.45, sx*0.4 + sx*0.2, sy*0.45 + sy*0.08, tocolor(255, 255, 255, 255 * a), 0.7/myX*sx, fonts["condensed-12"], "center", "center", false, false, false, true)
    ...
    if panelType == "normal" then 
        ...
        if getElementData(shopElement, "shop:private") == 1 then 
            cache_items = getElementData(shopElement, "shop:ped:items")
    ...
    if getElementData(shopElement, "shop:owner") == getElementData(localPlayer, "char:id") and getElementData(shopElement, "shop:private") == 1 then
        if panelType == "normal" then 
        ...
        elseif panelType == "admin" then 
            if (not orderPanelShowing) and (not priceSetPanelShowing) then 
                items = getElementData(shopElement, "shop:ped:items")
                ...
                dxDrawText("Egyenleg: "..color..(getElementData(shopElement, "shop:money")).."$", sx*0.5005, sy*0.49, sx*0.5005 + sx*0.197, sy*0.49 + sy*0.02, tocolor(255, 255, 255, 255*a), 0.9/myX*sx, fonts["condensed-10"], "center", "center", true, false, false, true)
...
function shopKey(key, state)
    if state then 
        ...
        if key == "mouse1" then 
            if getElementData(shopElement, "shop:private") == 1 then 
                if getElementData(shopElement, "shop:owner") == 0 then 
                    if core:isInSlot(sx*0.404, sy*0.51, sx*0.095, sy*0.03) then 
                        if getElementData(localPlayer, "char:money") >= getElementData(shopElement, "shop:cost") then 
                            outputChatBox(core:getServerPrefix("green-dark", "Bolt", 2).."Sikeresen megvásároltad a boltot!", 255, 255, 255, true)
            ...
            if getElementData(shopElement, "shop:owner") == getElementData(localPlayer, "char:id") then
                    ...
                    if (not orderPanelShowing) and (not priceSetPanelShowing) then
                        ...
                        if core:isInSlot(sx*0.5255, sy*0.565, sx*0.15, sy*0.023) then 
                            ...
                            if (editboxMoneyValue or 0) > 0 then 
                                if editboxMoneyValue <= getElementData(shopElement, "shop:money") then 
            ...
            if (getElementData(localPlayer, "user:admin") >= 6 and getElementData(localPlayer, "user:aduty")) and getElementData(shopElement, "shop:owner") == 0 then 
                if core:isInSlot(sx*0.6-40/myX*sx+2/myX*sx, sy*0.67+2/myY*sy, 36/myX*sx, 36/myY*sy) then 
...
function renderOrderPanel()
    ...
    for i = 1, 4 do
        ...
        if allOrder[i + orderPointer] then
            dxDrawImage(sx*0.35 + sx*0.005 + 3/myX*sx, startY + 2.5/myX*sx, 40/myX*sx, 40/myX*sx, inventory:getItemImage(allOrder[i + orderPointer][3]), 0, 0, 0, tocolor(255, 255, 255, 255 * orderPanelAlpha))
            dxDrawText("Bolt neve: "..color..(allOrder[i + orderPointer][2]), sx*0.35 + sx*0.035, startY + 2.5/myX*sx, sx*0.35, startY + sy*0.03, tocolor(255, 255, 255, 255 * orderPanelAlpha), 0.9/myX*sx, fonts["condensed-10"], "left", "center", false, false, false, true)
            dxDrawText("Mennyiség: "..color..(allOrder[i + orderPointer][4]).."#ffffffdb", sx*0.35 + sx*0.035, startY + 2.5/myX*sx, sx*0.35, startY + sy*0.045, tocolor(255, 255, 255, 255 * orderPanelAlpha), 0.9/myX*sx, fonts["condensed-10"], "left", "bottom", false, false, false, true)
            ...
            for i = 1, 4 do
                if core:isInSlot(sx*0.582 + sx*0.005 + 3/myX*sx, startY + 2.5/myX*sx, sx*0.05, 40/myX*sx) then 
                    ...
                    if pickupVeh then 
                        ...
                        if (inTransportID == 0) or (inTransportID == allOrder[i + orderPointer][1]) then
                            if #inTransportBoxes < 4 then 

                                if not isElement(deliveryBlip) then 
                                    for k, v in pairs(playerAllShopTable) do 
                                        if getElementData(v, "shop:id") == allOrder[i + orderPointer][1] then 
                                            local x, y, z = getElementPosition(getElementData(v, "shop:deliveryArea"))

                                            deliveryBlip = createBlip(x, y, z, 5)
                                            setElementData(deliveryBlip, "blip:name", allOrder[i + orderPointer][2])
                                ...
                                triggerServerEvent("shop > business > takeOrder", resourceRoot, pickupVeh, allOrder[i + orderPointer])
                                closeOrderPanel()
                                orderPointer = 0
                            else
    ...
    if getElementData(localPlayer, "user:admin") >= 4 then 
        ...
        if #nearbyShops >= 1 then 
            ...
            for k, v in ipairs(nearbyShops) do 
                ...
                if getElementData(v, "shop:private") == 1 then 
                    type = "Magán"
                    outputChatBox(color.."ID:#ffffff "..( getElementData(v, "shop:id") or 0)..color.." Név:#ffffff "..( getElementData(v, "ped:name") or "nan" )..color.." Típus: #ffffff"..type..color.." Tulajdonos: #ffffff"..getElementData(v, "shop:owner")..color.." Bolt egyenlege: #ffffff"..getElementData(v, "shop:money").."$", 255, 255, 255, true)
                else
            ...
            for k, v in pairs(playerAllShopTable) do 
                if isElement(v) then
                    ...
                    if isElement(col) then
                        local pos = getElementData(col, "deliveryArea:position")
