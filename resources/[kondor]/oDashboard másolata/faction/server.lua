...
function loadFactions()
    ...
    for k, v in ipairs(result) do 
        ...
        local dutyPos = fromJSON(v["dutyPos"])
...
addEventHandler("faction > setMemberDutyState", resourceRoot, function(dutyState, dutySkin)
    ...
    if dutyState == 0 then 
        ...
        for k2, v2 in pairs(server_factionMembers_list[oldDutyState]) do 
            if v2[1] == charID then 
                --print(k2)
                server_factionMembers_list[oldDutyState][k2][9] = (server_factionMembers_list[oldDutyState][k2][9] or 0) + (getElementData(client, "faction:duty:time") or 0)
